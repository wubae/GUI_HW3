...
function generateDynamicTable() {
    ...
    if (isNaN(vStart) || isNaN(vEnd)) {
        ...
        if (vStart < minAllowed || vEnd > maxAllowed) {
            errorMessages.push(`Column/row values must be between ${minAllowed} and ${maxAllowed}.`);
        }
        if (vStart > vEnd) {
            errorMessages.push('Column/row start value must be less than or equal to the end value.');
        }
    }
